...
__RW uint8_t buff_SALIDA = 0;
void GPIO_DebounceSALIDA(void) {
	static uint8_t q = 0;	//Quantity of bounces
	uint8_t j = 0;			//It captures changes

	if (GPIO_GetPIN(0, 1, ACT_LOW))	// The key is pushed?
		j = 0x01;				//Something is happening, the key is been pushed

	if (buff_SALIDA ^ j) {			// If the key is pushed while q != BOUNCE
		q++;						// I change the buffer
		if (q == BOUNCE) {
			q = 0;
			buff_SALIDA ^= 0x01;
...
__RW uint8_t buff_ENTRADA = 0;
void GPIO_DebounceENTRADA(void) {
	static uint8_t q = 0;	//Quantity of bounces
	uint8_t j = 0;			//It captures changes

	if (GPIO_GetPIN(0, 0, ACT_LOW))	// The key is pushed?
		j = 0x01;				//Something is happening, the key is been pushed

	if (buff_ENTRADA ^ j) {			// If the key is pushed while q != BOUNCE
		q++;						// I change the buffer
		if (q == BOUNCE) {
			q = 0;
			buff_ENTRADA ^= 0x01;
